...
internal sealed class SemestersPaginationFilteredFacade(SemesterSchema semester, int page, int pageSize) : IFacade<IReadOnlyCollection<SemesterResponse>>
{
	...
	public async Task<ActionResult<IReadOnlyCollection<SemesterResponse>>> Process()
	{
		...
		IService<IReadOnlyCollection<Semester>> service = new GetPagedSemestersByFilterService(_page, _pageSize, repository, expression);
