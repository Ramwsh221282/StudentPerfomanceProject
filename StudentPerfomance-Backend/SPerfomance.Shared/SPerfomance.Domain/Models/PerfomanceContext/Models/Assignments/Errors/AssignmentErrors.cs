...
{
	public static Error InvalidAssignmentValue(int value) => new Error($"Оценка {value} недопустима в контексте");
	...
	public static Error NotExist(Student student, SemesterPlan discipline) =>
		new Error($"Пара студент/дисциплина {student.Name} ${discipline.Discipline.Name} не найдены в этом списке контрольной недели");
}
