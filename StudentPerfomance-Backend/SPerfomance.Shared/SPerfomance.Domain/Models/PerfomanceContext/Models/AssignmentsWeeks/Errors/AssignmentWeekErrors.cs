...
public static class AssignmentWeekErrors
{
	...
	public static Error NotValidGroupContract(StudentGroup group) =>
		new Error($"Невозможно начать контрольную неделю, потому что у группы {group.Name} не назначен учебный план");

	public static Error TeacherAssignmentRequired(Semester semester, SemesterPlan plan) =>
		new Error($"Невозможно начать контрольную неделю, потому что у семестра {semester.Number} учебного плана {semester.Plan.Direction.Name.Name} {semester.Plan.Direction.Type.Type} имеется дисциплина {plan.Discipline.Name} без назначенного преподавателя");

	public static Error AssignmentAlreadyExists(Teacher teacher, SemesterPlan plan, Student student) =>
		new Error($"Проставление\nпреподаватель: {teacher.Name}\nдисциплина: {plan.Discipline}\nстудент: {student.Name}\nуже существует");
