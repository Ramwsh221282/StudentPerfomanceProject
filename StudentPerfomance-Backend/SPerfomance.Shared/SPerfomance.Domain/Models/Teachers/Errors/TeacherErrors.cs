...
public static class TeacherErrors
{
	...
	public static Error NameExceess(int length) => new Error($"Имя преподавателя более {length} символов");

	public static Error SurnameExceess(int length) => new Error($"Фамилия преподавателя более {length} символов");

	public static Error PatronymicExceess(int length) => new Error($"Отчество преподавателя более {length} символов");
	...
	public static Error WorkingStateEmpty() => new Error("Условие работы преподавателя было пустым");

	public static Error WorkingStateInvalid(string state) => new Error($"Условие работы {state} недопустимо");
