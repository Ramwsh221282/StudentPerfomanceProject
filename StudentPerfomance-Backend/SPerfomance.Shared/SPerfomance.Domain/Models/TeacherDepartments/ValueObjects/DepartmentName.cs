...
public class DepartmentName : DomainValueObject
{
	...
	public static Result<DepartmentName> Create(string name)
	{
		if (string.IsNullOrWhiteSpace(name))
			return Result<DepartmentName>.Failure(TeacherDepartmentErrors.DepartmentNameEmptyError());

		if (name.Length > MAX_NAME_LENGTH)
			return Result<DepartmentName>.Failure(TeacherDepartmentErrors.DepartmentNameExceessError(MAX_NAME_LENGTH));

		if (name.Length < MIN_NAME_LENGTH)
			return Result<DepartmentName>.Failure(TeacherDepartmentErrors.DepartmentNameLessError(MIN_NAME_LENGTH));
