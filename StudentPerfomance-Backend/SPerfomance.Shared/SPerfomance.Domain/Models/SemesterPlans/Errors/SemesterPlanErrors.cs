...
{
    public static Error TeacherAlreadyAttacher() =>
        new Error("Для этой дисциплины уже задан преподаватель");
    ...
    public static Error DisciplineDublicate() =>
        new Error("Дисциплина уже существует в этом семестре");
    ...
    public static Error TeacherHasDisciplineAlready(Teacher teacher, SemesterPlan plan) =>
        new Error(
            $"Преподаватель {teacher.Name.Name} {teacher.Name.Surname} {teacher.Name.Patronymic} уже ведет дисциплину {plan.Discipline.Name}"
        );

    public static Error TeacherDoesntChargeDiscipline(Teacher teacher) =>
        new Error(
            $"Преподаватель {teacher.Name.Name} {teacher.Name.Surname} {teacher.Name.Patronymic} не ведет эту дисциплину"
        );

    public static Error DoesntBelongSemester() =>
        new Error("Дисциплина не принадлежит этому семестру");
}
