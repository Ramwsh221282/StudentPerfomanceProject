...
public class EducationPlan : AggregateRoot
{
	...
	private EducationPlan(PlanYear year, EducationDirection direction) : base(Guid.NewGuid())
	{
		Year = year;
		Direction = direction;
		CreateSemesters();
	...
	public Result<StudentGroup> RemoveStudentGroup(StudentGroup group)
	{
		if (_groups.Any(g => g.Name == group.Name) == false)
			return Result<StudentGroup>.Failure(StudentGroupErrors.StudentGroupDoesntBelongEducationPlan(this, group));
