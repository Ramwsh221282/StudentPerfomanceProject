...
public static class EducationPlanErrors
{
	...
	public static Error DublicateError(EducationDirection direction, int year) =>
		new Error($"Учебный план {direction.Code.Code} {direction.Name.Name} {year} уже существует");

	public static Error NotFoundError() => new Error("Учебный план не найден");

	public static Error WithoutDirection() => new Error("Учебный план нельзя создать без указания направления подготовки");
