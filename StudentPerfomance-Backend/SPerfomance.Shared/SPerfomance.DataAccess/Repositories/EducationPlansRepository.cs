...
public class EducationPlansRepository : IEducationPlansRepository
{
	...
	public async Task Update(EducationPlan entity) =>
		await _context.EducationPlans.Where(p => p.Id == entity.Id)
		.ExecuteUpdateAsync(
	...
	public async Task<IReadOnlyCollection<EducationPlan>> GetFiltered(string? directionName, string? directionCode, string? directionType, int? year) =>
			await _context.EducationPlans
			.Include(p => p.Direction)
			.Include(p => p.Semesters)
			.ThenInclude(s => s.Disciplines)
			.ThenInclude(c => c.Teacher)
			.Where(p =>
			!string.IsNullOrWhiteSpace(directionName) && p.Direction.Name.Name.Contains(directionName) ||
			!string.IsNullOrWhiteSpace(directionCode) && p.Direction.Code.Code.Contains(directionCode) ||
			!string.IsNullOrWhiteSpace(directionType) && p.Direction.Type.Type.Contains(directionType) ||
			year == p.Year.Year)
			.AsNoTracking()
			.AsSplitQuery()
			.ToListAsync();
