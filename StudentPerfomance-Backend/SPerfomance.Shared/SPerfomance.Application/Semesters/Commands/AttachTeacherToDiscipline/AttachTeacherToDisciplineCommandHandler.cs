...
public class AttachTeacherToDisciplineCommandHandler
(
...
) : ICommandHandler<AttachTeacherToDisciplineCommand, SemesterPlan>
{
	...
	public async Task<Result<SemesterPlan>> Handle(AttachTeacherToDisciplineCommand command)
	{
		...
		Result<SemesterPlan> plan = command.Discipline.Semester.AttachTeacherToDiscipline(command.Discipline, command.Teacher);
		if (plan.IsFailure)
			return plan;
