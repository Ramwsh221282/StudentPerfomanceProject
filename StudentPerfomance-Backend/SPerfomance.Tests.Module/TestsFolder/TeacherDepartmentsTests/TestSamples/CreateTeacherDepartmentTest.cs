...
internal sealed class CreateTeacherDepartmentTest(DepartmentSchema schema) : IService<TeachersDepartment>
{
	...
	public async Task<OperationResult<TeachersDepartment>> DoOperation()
	{
		...
		IService<TeachersDepartment> service = new TeacherDepartmentCreationService
		(
			_schema,
			expression,
			repository
		...
		new(result, "Department Creation Log Info:");
		logger.ShowInfo();
