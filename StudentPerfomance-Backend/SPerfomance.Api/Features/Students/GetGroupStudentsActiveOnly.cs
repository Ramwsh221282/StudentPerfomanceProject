...
public static class GetGroupStudentsActiveOnly
{
	...
	public sealed class Endpoint : IEndpoint
	{
		public void MapEndpoint(IEndpointRouteBuilder app) =>
			app.MapPost($"{StudentTags.Api}/by-group-active-only", Handler).WithTags($"{StudentTags.Tag}");
	}
	...
	public static async Task<IResult> Handler([FromBody] Request request, IStudentGroupsRepository groups, IUsersRepository users)
	{
		...
		return Results.Ok(group.Value.Students.Where(s => s.State == StudentState.Active).Select(s => s.MapFromDomain()));
	}
}
