...
public static class GetUsersByPage
{
	...
	public async static Task<IResult> Handler(Request request, IUsersRepository repository
	)
	{
		...
		IReadOnlyCollection<User> users = await repository.GetPaged(request.Pagination.Page, request.Pagination.PageSize);
