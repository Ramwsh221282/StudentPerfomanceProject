...
{
	public record Request(StudentGroupContract Initial, StudentGroupContract Updated, TokenContract Token);
	...
	)
	{
		if (await repository.HasWithName(request.Updated.Name.ValueOrEmpty()))
			return Results.BadRequest(StudentGroupErrors.NameDublicate(request.Updated.Name.ValueOrEmpty()).Description);

		if (!await new UserVerificationService(users).IsVerified(request.Token, UserRole.Administrator))
			return Results.BadRequest(UserTags.UnauthorizedError);

		Result<StudentGroup> group = await new GetStudentGroupQueryHandler(repository).Handle(request.Initial);
		group = await new ChangeGroupNameCommandHandler(repository).Handle(new(group.Value, request.Updated.Name.ValueOrEmpty()));
