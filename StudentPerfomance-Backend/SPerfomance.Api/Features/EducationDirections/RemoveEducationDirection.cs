...
public static class RemoveEducationDirection
{
	...
	public sealed class Endpoint : IEndpoint
	{
		public void MapEndpoint(IEndpointRouteBuilder app) =>
			app.MapDelete($"{EducationDirectionTags.Api}", Handler).WithTags(EducationDirectionTags.Tag);
	}
	...
	public static async Task<IResult> Handler([FromBody] Request request, IUsersRepository users, IEducationDirectionRepository repository)
	{
		...
		direction = await new RemoveEducationDirectionCommandHandler(repository)
		.Handle(new(direction.Value));
