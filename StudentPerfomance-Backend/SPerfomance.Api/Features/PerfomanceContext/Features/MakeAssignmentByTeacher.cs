...
public static class MakeAssignmentByTeacher
{
	...
	)
	{
		...
		MakeAssignmentCommand command = new MakeAssignmentCommand(request.Assignment.Id, request.Assignment.Mark);
		Result<StudentAssignment> assignment = await new MakeAssignmentCommandHandler(sessions, assignments).Handle(command);
