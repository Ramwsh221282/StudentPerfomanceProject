...
public static class CreateAssignmentSession
{
	...
	public sealed class Endpoint : IEndpoint
	{
		public void MapEndpoint(IEndpointRouteBuilder app) =>
			app.MapPost($"{PerfomanceContextTags.SessionsApi}", Handler).WithTags($"{PerfomanceContextTags.SessionsTag}");
	}
		...
		)
	{
		...
		Result<AssignmentSession> session = await new CreateAssignmentSessionCommandHandler(sessions, groups)
