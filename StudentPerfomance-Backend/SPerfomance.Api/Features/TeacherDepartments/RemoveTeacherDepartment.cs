...
public static class RemoveTeacherDepartment
{
	...
	public sealed class Endpoint : IEndpoint
	{
		public void MapEndpoint(IEndpointRouteBuilder app) =>
			app.MapDelete($"{TeacherDepartmentsTags.Api}", Handler).WithTags(TeacherDepartmentsTags.Tag);
	}

	public static async Task<IResult> Handler([FromBody] Request request, IUsersRepository users, ITeacherDepartmentsRepository repository)
	{
		if (!await new UserVerificationService(users).IsVerified(request.Token, UserRole.Administrator))
			return Results.BadRequest(UserTags.UnauthorizedError);

		Result<TeachersDepartments> department = await new GetDepartmentByNameQueryHandler(repository).Handle(request.Department);
		department = await new RemoveTeachersDepartmentCommandHandler(repository).Handle(new(department.Value));
