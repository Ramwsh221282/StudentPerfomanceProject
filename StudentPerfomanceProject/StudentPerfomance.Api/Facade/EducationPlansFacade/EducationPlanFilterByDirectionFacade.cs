...
internal sealed class EducationPlanFilterByDirectionFacade(EducationPlanGeneralRequest request, int page, int pageSize) : IFacade<IReadOnlyCollection<EducationPlanResponse>>
{
	...
	public async Task<ActionResult<IReadOnlyCollection<EducationPlanResponse>>> Process()
	{
		...
		EducationDirectionRepositoryParameter parameter = EducationDirectionSchemaConverter.ToRepositoryParameter(plan.Direction);
		IRepositoryExpression<EducationPlan> filter = EducationPlanExpressionsFactory.CreateFilterByDirection(parameter);
