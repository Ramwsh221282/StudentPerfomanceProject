...
public sealed class GetFilteredEducationPlansTest(EducationPlanGeneralRequest request) : IService<IReadOnlyCollection<EducationPlan>>
{
	...
	public async Task<OperationResult<IReadOnlyCollection<EducationPlan>>> DoOperation()
	{
		...
		new(result, "Education plan filter test");
		logger.ShowInfo();
		if (result.Result != null && result.Result.Count > 0)
		{
			Console.WriteLine("Education plan filter test info:");
