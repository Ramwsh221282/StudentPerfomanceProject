...
public class FilterPagedByCodeEducationDirectionTest(FilterEducationDirectionRequest request, int page, int pageSize) : IService<IReadOnlyCollection<EducationDirection>>
{
	...
	public async Task<OperationResult<IReadOnlyCollection<EducationDirection>>> DoOperation()
	{
		...
		(
			new FilterConstraint(FilterConstraint.CodeOnly),
			_page,
			_pageSize,
			_repository,
			expression
		...
		new(result, "Education Direction Paged Filter By Code Test");
		logger.ShowInfo();
