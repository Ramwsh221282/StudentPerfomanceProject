...
public sealed class FilterEducationDirectionByCodeTest(FilterEducationDirectionRequest request) : IService<IReadOnlyCollection<EducationDirection>>
{
	...
	public async Task<OperationResult<IReadOnlyCollection<EducationDirection>>> DoOperation()
	{
		...
		IService<IReadOnlyCollection<EducationDirection>> service = EducationDirectionGetCollectionByFilterBuilder.Build(new FilterConstraint(FilterConstraint.CodeOnly), _repository, expression);
		...
		new(result, "Education Direction By Code Filter Request Test");
		logger.ShowInfo();
