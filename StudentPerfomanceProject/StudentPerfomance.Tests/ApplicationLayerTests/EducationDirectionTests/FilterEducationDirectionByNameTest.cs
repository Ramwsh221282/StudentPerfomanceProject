...
public sealed class FilterEducationDirectionByNameTest(FilterEducationDirectionRequest request) : IService<IReadOnlyCollection<EducationDirection>>
{
	...
	public async Task<OperationResult<IReadOnlyCollection<EducationDirection>>> DoOperation()
	{
		...
		IService<IReadOnlyCollection<EducationDirection>> service = EducationDirectionGetCollectionByFilterBuilder.Build(new FilterConstraint(FilterConstraint.NameOnly), _repository, expression);
		...
		new(result, "Education Direction By Name Filter Request Test");
		logger.ShowInfo();
