...
public sealed class FindEducationPlanExpression
(
	EducationPlanRepositoryParameter plan,
	EducationDirectionRepositoryParameter direction
) : IRepositoryExpression<EducationPlan>
{
	...
	public Expression<Func<EducationPlan, bool>> Build() =>
		(EducationPlan entity) =>
			...
			entity.Direction.Name.Name == _direction.Name &&
			entity.Direction.Code.Code == _direction.Code &&
			entity.Direction.Type.Type == _direction.Type;
}
